export interface FinancialRecord {
  ...
  effectiveDate: string|null; sourceIds: string[]; parentFundingId?: string; securityId?: string; overlapsRecordIds?: string[]; assumption?: string;
}
export type MeasureKind = 'direct-evidence'|'historical-comparison'|'mixed-funding'|'same-day-holdings';
...
export function assembleMeasure(records: FinancialRecord[], ids: string[], kind: MeasureKind) {
  ...
  const eligible = kind==='mixed-funding'?['holding','purchase','derived-residual','funding']:kind==='historical-comparison'?['holding','purchase','derived-residual']:kind==='same-day-holdings'?['holding']:['holding','purchase'];
  ...
  for(const row of selected){
    ...
    if(!row.sourceIds.length||!row.effectiveDate||!/^\d{4}-\d{2}-\d{2}$/.test(row.effectiveDate)) throw new Error('A dated source is required');
    if(row.stage==='derived-residual'&&!row.assumption)throw new Error('A derived residual needs an explicit assumption');
    ...
    if(['direct-evidence','historical-comparison'].includes(kind)&&row.basis!=='par')throw new Error('Security comparisons use par, not market value or manager funding');
  }
  ...
  const label=kind==='mixed-funding'?'Historical illustration including full manager funding':kind==='historical-comparison'?'Mixed-date historical calculation including an estimated residual':kind==='same-day-holdings'?`Observed holdings on ${dates[0]}`:`Documented securities${dates.length>1?' from different record dates':''}`;
